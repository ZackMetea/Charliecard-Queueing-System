<?php
	...
	$sql = "Select 
		...
		Where 
		(status in (1,2,6) ) and
		firstname LIKE '%" . mysql_escape_string(trim($_GET['firstName'])) ."%' and  
		lastname LIKE '%" . mysql_escape_string(trim($_GET['lastName'])) ."%' and  
		email LIKE '%" . mysql_escape_string(trim($_GET['email'])) ."%' and 
		phonenumber LIKE '%" . mysql_escape_string(trim($_GET['phone'])) ."%' 
	...
	while ($stmt->fetch()){
		...
		echo "<td><input type=\"button\" name=\"printcard\" style=\"border: none; display: table-cell; width: 100%\" value=\"Edit Record\" onclick=\"createFormFunction('row".$count."', 'Record')\" /></td>\n";
		echo "</tr>\n";
	}
	echo "</table>\n";
	$conn->close();
